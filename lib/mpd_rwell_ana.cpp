...
std::pair<double, double> PadRange(const Range &r, double defMin, double defMax)
{
  if (!r.set) {
...
MpdRwellAna::MpdRwellAna(MpdRwellAnaOptions opt)
    : opt_(std::move(opt))
{
}
...
int MpdRwellAna::Run()
{
  const TString inFile = opt_.input.c_str();
  const TString outPdf = opt_.output.c_str();
  ...
  const Long64_t totalEntries = tr.GetTree()->GetEntries();
  const Long64_t scanEntries = (totalEntries > 0)
                                   ? std::max<Long64_t>(1, totalEntries / 10)
                                   : 0;
  Long64_t scanned = 0;
  while (scanned < scanEntries && tr.Next()) {
  ...
  if (opt_.avgEvents > 0) {
    ...
    if (trEvt.GetTree()) {
      ...
      int shown = 0;
      while (trEvt.Next() && shown < opt_.avgEvents) {
        const int sf = *sfirst;
        const int sd = *sdelta;
  ...
  c.Print(Form("%s]", outPdf.Data()));
  std::cout << "Output PDF: " << RelPathToSuite(outPdf.Data()) << "\n";
  return 0;
}
